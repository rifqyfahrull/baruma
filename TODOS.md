...
- [ ] **Bug Rules-of-Hooks nyata di `plan-canvas.tsx`** (WS-A 2026-08-23): 4 pemanggilan `useEditorStore` (~baris 1867–1879) berada SETELAH `if (!layout || !site) return null` — pelanggaran Rules of Hooks asli (bukan gaya), berpotensi crash "Rendered fewer hooks than expected" bila `layout`/`site` beralih null↔non-null antar render. CI lint sekarang menegakkan `eslint` sebagai gate wajib (`.github/workflows/ci.yml`), jadi keempat baris ini + 7 pelanggaran `react-hooks/set-state-in-effect` (setState sinkron dalam effect, respons ke nonce eksternal) di `src/app/app/projects/[projectId]/editor/page.tsx`, `src/app/auth/handoff/page.tsx`, `src/components/admin/template-form.tsx`, `src/components/editor/editor-warnings-panel.tsx`, `src/components/layout/floating-panel.tsx`, `src/components/preview-3d/preview-3d-view.tsx` DITANDAI `eslint-disable-next-line` bertanggal (bukan diperbaiki) — memperbaikinya butuh verifikasi visual kanvas 2D/3D (Playwright) di luar scope WS-A. Cari `PRA-EXISTING (bukan dari WS-A)` di kode untuk daftar lengkap lokasi.
